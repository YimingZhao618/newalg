    ...
    if config_name is not None:
        with open(os.path.join(os.path.dirname(__file__), "config", subfolder, "{}.yaml".format(config_name)),
            ...
            try:
                ...
                config_dict = yaml.safe_load(f)##这里是经典错误，PyYAML 5.1 之后弃用了不带 Loader 参数的 yaml.load(f) 用法。现在的 yaml.load 函数必须显式指定加载器（Loader），或者使用更安全的 safe_load。
...
def recursive_dict_update(d, u):
    for k, v in u.items():
        ...
        if isinstance(v, collections.abc.Mapping):#在 Python 3.10 中，Mapping 等抽象基类已经被彻底从 collections 模块移除，必须使用 collections.abc
